#!/usr/bin/env python3
"""Generate all entity, repository, DTO, service, and controller Java files."""
import os

ROOT = os.path.dirname(os.path.abspath(__file__))
BE = os.path.join(ROOT, "backend/src/main/java/com/taevas/clinic")
...
# ENTITIES
# ============================
def gen_entity(name, table, fields, extra_imports=""):
    ...
    field_lines = []
    for col, jtype, jname in fields:
        nullable = "nullable = false" if col.endswith("!") else ""
        col = col.rstrip("!")
        unique = ""
        if col.endswith("~"):
            unique = ", unique = true"
            col = col.rstrip("~")
        ann = f'@Column(name = "{col}"'
        if nullable:
            ann += f", {nullable}"
        if unique:
            ann += unique
        ann += ")"
        field_lines.append(f"    {ann} private {jtype} {jname};")
    body = "\n".join(field_lines)
    return f"""package com.taevas.clinic.model;
...
entities = [
    ...
    ("InvoiceItem", "invoice_items", [
        ("invoice_id!", "UUID", "invoiceId"),
        ("description!", "String", "description"), ("quantity", "Integer", "quantity"),
        ("unit_price!", "BigDecimal", "unitPrice"), ("amount!", "BigDecimal", "amount"),
    ...
    ("ClinicBillingConfig", "clinic_billing_config", [
        ...
        ("payment_modes", "String", "paymentModes"),
    ], "import java.math.BigDecimal;"),
    ...
    ("WhatsAppBotConfig", "whatsapp_bot_config", [
        ...
        ("welcome_message", "String", "welcomeMessage"),
        ("auto_reply", "Boolean", "autoReply"),
    ], ""),
...
print("=== ENTITIES ===")
for name, table, fields, extra in entities:
    write(os.path.join(BE, "model", f"{name}.java"), gen_entity(name, table, fields, extra))
...
print("\n=== REPOSITORIES ===")
for rname, (ename, methods) in repos.items():
    content = f"""package com.taevas.clinic.repository;
...
"""
    write(os.path.join(BE, "repository", f"{rname}.java"), content)

print(f"\nDone: {len(entities)} entities, {len(repos)} repositories")
